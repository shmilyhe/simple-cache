...
import java.util.concurrent.Executors;
    ...
    public class LRUCache<K, V> implements Cache<K, V> {
    	...
    	public static void addWacher(final LRUCache cache){
    		fixedThreadPool.submit(new Callable(){
				...
				public Object call() throws Exception {
					...
					cache.recycle(System.currentTimeMillis());
					//System.out.println("======================================");
					return null;
				}
    	...
    	/**
    	 * 获取所有的值
    	 * @return collection of active item
    	 */
		...
		private  Collection<CacheNode<K, V>> allNodes() {
    		ArrayList<CacheNode<K, V>> al = new ArrayList<CacheNode<K, V>>();
    		CacheNode<K, V> flag = head;
    		while (flag != null) {
    			al.add(flag);
    	 ...
    	 * 回收超时的
    	 */
    	public void recycle(long tid ){
    		try{
				tranId=tid;
	    		Collection<CacheNode<K, V>> nodes = this.allNodes();
				CacheNode<K, V> lastActive=null;
	    		...
	    		for(CacheNode<K, V> n :nodes){
					...
					isFirst=false;
					//System.out.println("*************************"+n.key+"|"+n.isExpire());
	    		}
    		}catch(Throwable t){
    			t.printStackTrace();
    		}
		...
		IInactive inactiveHandler;
		public void onInactive(IInactive handler){
			inactiveHandler=handler;
		}

		private void doInactive(K k,V v,long create){
			try{
